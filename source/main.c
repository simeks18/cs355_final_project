...
int main(int argc, char* argv[])
{
	...
	headNode = get_directory_information(dirpath);
    ...
    int ch;
    int count = 0;
	file_info* it = headNode;
    for (file_info* it = headNode; it->number > 0; it = it->next) 
		count++;
    ...
    while (1) 
	{
        ...
        int row = 3;
        int idx = 0;
        for (file_info* it = headNode; it->number > 0; it = it->next, ++idx) 
		{
            if (idx == highlight) 
			{
                attron(A_REVERSE);
